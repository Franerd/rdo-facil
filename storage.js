  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title: `RDO — ${report.obra || dateBR(report.data)}`,
        text,
      });
      return;
    } catch (error) {
...
function renderHistory() {
  ...
  list.innerHTML = reports
    .map(
      (report) =>
        `<article class="historyItem"><div><strong>${escapeHtml(report.obra || "Obra não informada")}<span class="statusPill ${report.status === "closed" ? "closed" : ""}">${report.status === "closed" ? "Concluído" : "Em andamento"}</span></strong><small>${escapeHtml(dateBR(report.data))} · ${escapeHtml(report.local || "Local não informado")}</small></div><div class="historyActions"><button data-share="${report.id}">Compartilhar</button><button data-open="${report.id}">Abrir</button><button data-copy="${report.id}">Duplicar</button><button class="delete" data-delete="${report.id}">Excluir</button></div><details><summary>Consultar log (${report.log.length})</summary><ul class="logList">${report.log
          .slice()
          .reverse()
          .map(
            (item) =>
...
document.getElementById("historico").onclick = (event) => {
  ...
  if (button.dataset.share && report) {
    shareReport(report);
    return;
  }
